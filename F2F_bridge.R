...
"If no database is available to provide weights to the F2F plot function, 
in house generated laboratory data can be used to search for relationships
between In-Silico features and protein performance, which can then be used 
to predict performance of future proteins." 

lasso_select = function(data, results){
  "data is the same input data as used for input to main F2F plot function
  results is a vector containing wet lab results, in the same order as the data table"
  input_lr = data[-c(1:3),]
  train = results
  library(glmnet)
